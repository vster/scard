...
LONG sc_selectfile (sc_context *ctx, DWORD fID)
{
  ...
  BYTE apdu[] = "\x00\xa4\x00\x00\x02\x3f\x00";

  int sz = sizeof(apdu) - 1;
  memcpy(buf,apdu,sz);
...
LONG sc_readdata (sc_context *ctx, BYTE datsz, DWORD offs)
{
  ...
  BYTE apdu[] = "\x00\xb0\x00\x00\x00";

  int sz = sizeof(apdu) - 1;

  for (i = 0; i < (int) dw; i++) buf[i] = 0x00;
...
DWORD dw=sizeof(ctx->sw);
BYTE *buf= ctx->sw;
BYTE apdu[]=// TAG 83 - file name, 86 - ACL
...
#endif
  //
  BYTE apdu[]=// TAG 80 - file size, 83 - file name, 86 - ACL
             "\x00\xE0\x00\x00\x1f\x62\x1d"
...
//
LONG
sc_selectdir(sc_context *ctx, DWORD id)
{
//select dir
LONG  rc=SCARD_S_SUCCESS;
#if _DEBUG_
...
BYTE apdu[]=//
  ...
  //
#if _DEBUG_
  printf("SELECT DF %04X APDU Length %d\n",id,sz);
  for(i=0;i<sz;i++)
	printf("%02X ",buf[i]);
  printf("\n");
#endif
...
#if _DEBUG_
  printf("SHOW CURRENT DIR ...\n");
#endif
  //
#if _DEBUG_
  printf("SHOW CURRENT DIR APDU Length %d\n", sz);
  for(i=0;i<sz;i++)
  printf("%02X ",buf[i]);
  printf("\n");
#endif
