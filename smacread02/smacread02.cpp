...
int main(int argc, char* argv[])
{
  ...
  if (rc == SC_OK)
  {
    ...
    printf ("\nWriting array of hex-codes '%02x' to smart card\nrc = %2x\n", code, rc);

    printf ("\nReading array of hex-codes '%02x' from smart card...\nPress Enter to stop reading. ", code);
...
//	while ( !(_kbhit() && _getch()==13)  )
	...
	szk = k * len;
	double speed = szk/seconds;
	printf("\nrc = %2x; cycles = %d; bytes read = %d \n", ctx.rw, k, szk);
	printf("time = %4.0f s; speed = %4.0f bytes/s \n", seconds, speed);
